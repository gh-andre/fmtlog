...
The are two script files in this repository, `get-fmt.bat`
...
references as a submodule (or compatible), builds it with
...
```
git diff v2.2.1 v2-2-1-patches
```
...
```
git diff v2.2.1 v2-2-1-patches -- CMakeLists.txt fmtlog.h fmtlog-inl.h
```
...
```
curl --location --output fmtlog-2.2.1.tar.gz \
    https://github.com/MengRao/fmtlog/archive/refs/tags/v2.2.1.tar.gz

tar -xzf fmtlog-2.2.1.tar.gz
cd fmtlog-2.2.1
patch --strip=1 --input=fmtlog-2.2.1.patch
```
